    ...
    >
      ...
      <nav className={`sticky top-0 z-50 border-b backdrop-blur-xl transition-colors duration-500 ${isDark ? "border-white/5 bg-[#080812]/80" : "border-slate-100 bg-white/90"}`}>
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-3 md:px-6 md:py-4">
          ...
          <div className="hidden items-center gap-8 md:flex">
            <a href="/learn" className={`text-sm font-medium transition ${isDark ? "text-white/60 hover:text-white" : "text-slate-600 hover:text-slate-900"}`}>
              학습하기
            </a>
            <a href="#features" className={`text-sm font-medium transition ${isDark ? "text-white/60 hover:text-white" : "text-slate-600 hover:text-slate-900"}`}>
              학습 기능
            </a>
            <a href="#how" className={`text-sm font-medium transition ${isDark ? "text-white/60 hover:text-white" : "text-slate-600 hover:text-slate-900"}`}>
              사용 방법
            </a>
            <a href="#faq" className={`text-sm font-medium transition ${isDark ? "text-white/60 hover:text-white" : "text-slate-600 hover:text-slate-900"}`}>
              FAQ
            </a>
          </div>
          ...
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <a href="/workspace"
              className={`hidden rounded-lg px-4 py-1.5 text-xs font-bold tracking-wide transition md:block lg:px-5 lg:py-2.5 ${isDark ? "bg-gradient-to-r from-blue-600 to-violet-600 text-white shadow-lg shadow-violet-900/40 hover:opacity-90" : "bg-blue-600 text-white shadow-lg shadow-blue-200 hover:bg-blue-700"}`}>
              START
            </a>
          </div>
        </div>
      ...
      <section className="relative z-10 overflow-hidden px-4 pb-24 pt-16 text-center md:pb-28 md:pt-36">
        ...
        <div className={`mb-6 inline-flex items-center gap-2 rounded-full border px-4 py-1.5 text-xs font-semibold tracking-wide ${isDark ? "border-white/10 bg-white/5 text-white/60 backdrop-blur" : "border-slate-200 bg-white text-slate-600 shadow-sm"}`}>
          <span className={`h-1.5 w-1.5 rounded-full ${isDark ? "animate-pulse bg-blue-400" : "bg-blue-500"}`} />
        ...
        <p className={`mx-auto mt-6 max-w-2xl text-xl font-medium md:text-2xl ${isDark ? "text-white/55" : "text-slate-500"}`}>
          비전공자도, 1일 만에, 진짜 프로토타입.
        </p>

        <p className={`mx-auto mt-3 max-w-2xl text-base leading-relaxed ${isDark ? "text-white/35" : "text-slate-400"}`}>
          AI와 대화하며 개발하는 바이브 코딩을 가장 쉽게 배웁니다.
          <br className="hidden md:block" />
          당신의 첫 웹앱, 여기서 시작하세요.
        </p>

        {/* ✅ 버튼 2개 - 하나는 workspace, 하나는 learn */}
        <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
          <a href="/workspace"
            className={`rounded-full px-8 py-3 text-sm font-bold uppercase tracking-wide transition sm:text-base ${isDark ? "bg-gradient-to-r from-blue-600 to-violet-600 text-white shadow-xl hover:scale-[1.02] hover:opacity-90" : "bg-blue-600 text-white shadow-xl hover:scale-[1.02] hover:bg-blue-700"}`}>
            바이브 코딩 시작하기 →
          </a>
          <a href="/learn"
            className={`rounded-full border px-8 py-3 text-sm font-bold uppercase tracking-wide transition sm:text-base ${isDark ? "border-white/10 bg-white/5 text-white/70 backdrop-blur hover:bg-white/10" : "border-slate-200 bg-white text-slate-700 shadow-sm hover:bg-slate-50"}`}>
            바이브 코딩 배우기
          </a>
        </div>

        {/* 채팅 미리보기 */}
        <div className={`mx-auto mt-20 max-w-2xl overflow-hidden rounded-[2rem] text-left transition-all duration-500 ${isDark ? "border border-white/10 bg-white/5 shadow-2xl shadow-black/50 backdrop-blur-xl" : "border border-slate-200 bg-white shadow-2xl shadow-slate-200"}`}>
          <div className={`flex items-center gap-2 border-b px-5 py-3 ${isDark ? "border-white/10" : "border-slate-100 bg-slate-50"}`}>
            ...
            <span className={`ml-2 text-xs font-semibold uppercase tracking-wider ${isDark ? "text-white/30" : "text-slate-400"}`}>
              VIBE PROJECT — AI 튜터
            </span>
          </div>
          ...
          <div className="space-y-4 p-6">
            <div className="flex flex-row-reverse gap-3">
              ...
              <div className={`rounded-2xl rounded-tr-sm px-4 py-3 text-sm ${isDark ? "bg-white/10 text-white/80" : "bg-slate-100 text-slate-700"}`}>
            ...
            <div className="flex gap-3">
              ...
              <div className={`max-w-sm rounded-2xl rounded-tl-sm px-4 py-3 text-sm text-white ${isDark ? "bg-gradient-to-br from-blue-600/80 to-violet-600/80" : "bg-blue-600"}`}>
                ...
                <p className={`mt-2 rounded-xl px-3 py-2 text-xs ${isDark ? "bg-white/10 text-white/80" : "bg-blue-700/50 text-blue-100"}`}>
                  "사용자들이 사진과 별점을 남길 수 있는 맛집 리스트 웹앱을 React + Tailwind로 만들어줘."
                </p>
                <p className={`mt-2 text-xs ${isDark ? "text-white/50" : "text-blue-200"}`}>
      ...
      {/* 통계 */}
      <section className={`relative z-10 border-y py-14 ${isDark ? "border-white/5 bg-white/[0.02] backdrop-blur" : "border-slate-100 bg-white"}`}>
        <div className="mx-auto grid max-w-6xl grid-cols-2 gap-8 px-6 md:grid-cols-4">
          {[
            ...
            <div key={stat.label} className="text-center">
              <div className={`text-3xl font-extrabold tracking-tight md:text-4xl ${isDark ? "bg-gradient-to-r from-blue-400 to-violet-400 bg-clip-text text-transparent" : "text-slate-900"}`}>
                {stat.value}
              </div>
        ...
        <div className="mx-auto max-w-6xl">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <h2 className="mt-3 text-3xl font-extrabold tracking-tight md:text-4xl">
              단순한 도구가 아닙니다.
              <br />
              <span className={isDark ? "bg-gradient-to-r from-blue-400 to-violet-400 bg-clip-text text-transparent" : ""}>
                당신의 첫 바이브 코딩 튜터입니다.
              </span>
            </h2>
          ...
          <div className="mt-16 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {features.map((feature) => (
              <div key={feature.title}
                className={`group relative overflow-hidden rounded-3xl border p-8 transition ${isDark ? "border-white/10 bg-white/5 backdrop-blur hover:border-white/20 hover:bg-white/10" : "border-slate-200 bg-white shadow-sm hover:border-blue-200 hover:shadow-lg"}`}>
                ...
                <div className={`mb-5 inline-flex h-10 w-10 items-center justify-center rounded-xl text-sm font-bold ${isDark ? `bg-gradient-to-br ${feature.gradient} text-white shadow-lg` : "bg-blue-50 text-blue-600"}`}>
                  {feature.number}
                </div>
          ...
          <div className={`mt-12 rounded-3xl border p-8 text-center ${isDark ? "border-white/10 bg-white/5" : "border-blue-100 bg-blue-50"}`}>
            <p className={`mb-2 text-lg font-bold ${isDark ? "text-white" : "text-slate-900"}`}>
              이 모든 걸 직접 배우고 싶으신가요?
            </p>
            <p className={`mb-6 text-sm ${isDark ? "text-white/40" : "text-slate-500"}`}>
              16개 레슨과 AI 튜터가 기다리고 있어요.
            </p>
            <a href="/learn"
      ...
      {/* 사용 방법 */}
      <section id="how" className={`relative z-10 px-6 py-24 ${isDark ? "" : "bg-slate-50"}`}>
        <div className="mx-auto max-w-6xl">
          <div className="mx-auto max-w-2xl text-center">
            ...
            <p className={`mt-4 text-base leading-7 ${isDark ? "text-white/40" : "text-slate-500"}`}>
              복잡한 이론 강의는 줄이고, 내 아이디어를 실제로 구현하는 과정 자체를 하나의 학습 경험으로 설계했습니다.
            </p>
          </div>
          ...
          <div className="mt-16 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {steps.map((step) => (
              <div key={step.number}
                className={`group relative overflow-hidden rounded-3xl border p-8 transition ${isDark ? "border-white/10 bg-white/5 backdrop-blur hover:border-white/20 hover:bg-white/10" : "border-slate-200 bg-white shadow-sm"}`}>
                ...
                <div className={`text-3xl font-extrabold ${isDark ? `bg-gradient-to-r ${step.gradient} bg-clip-text text-transparent` : "text-blue-600"}`}>
                  {step.number}
                </div>
      ...
      <section className="relative z-10 px-6 py-24">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-16 grid gap-4 md:grid-cols-3">
            {userTypes.map((user) => (
              <div key={user.title}
                className={`group relative overflow-hidden rounded-3xl border p-8 transition ${isDark ? "border-white/10 bg-white/5 backdrop-blur hover:border-white/20 hover:bg-white/10" : "border-slate-200 bg-white shadow-sm hover:shadow-lg"}`}>
      ...
      {/* FAQ */}
      <section id="faq" className={`relative z-10 px-6 py-24 ${isDark ? "" : "bg-slate-50"}`}>
          ...
          <div className="mt-16 space-y-3">
            {faqs.map((faq) => (
              <details key={faq.question}
                className={`rounded-2xl border px-6 py-5 transition ${isDark ? "border-white/10 bg-white/5 backdrop-blur hover:border-white/20" : "border-slate-200 bg-white"}`}>
                <summary className={`flex cursor-pointer list-none items-center justify-between font-semibold ${isDark ? "text-white" : "text-slate-900"}`}>
                  {faq.question}
                  <span className={`ml-4 shrink-0 ${isDark ? "text-white/30" : "text-slate-400"}`}>↓</span>
                </summary>
                <p className={`mt-4 text-sm leading-7 ${isDark ? "text-white/50" : "text-slate-500"}`}>{faq.answer}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <section id="cta" className="relative z-10 px-6 py-32 text-center">
        <div className={`relative mx-auto max-w-3xl overflow-hidden rounded-3xl px-8 py-20 ${isDark ? "border border-white/10 bg-white/5 backdrop-blur-xl" : "bg-slate-900 shadow-2xl"}`}>
          {isDark ? (
            <>
          ...
          <h2 className="relative text-3xl font-extrabold tracking-tight text-white md:text-5xl">
            당신의 첫 웹앱,
            <br />
            <span className={isDark ? "bg-gradient-to-r from-blue-400 via-violet-400 to-pink-400 bg-clip-text text-transparent" : "text-blue-400"}>
              여기서 시작하세요.
            </span>
          </h2>
          <p className="relative mx-auto mt-5 max-w-xl text-lg leading-8 text-white/60">
            막연했던 아이디어를 바이브 코딩으로 현실로 만들어보세요. 지금 바로 첫 프로토타입을 시작할 수 있습니다.
          </p>

          <div className="relative mt-10 flex flex-col items-center justify-center gap-4 sm:flex-row">
            <a href="/workspace"
              className={`rounded-xl px-8 py-4 text-base font-semibold text-white transition ${isDark ? "bg-gradient-to-r from-blue-600 to-violet-600 shadow-2xl shadow-violet-900/50 hover:scale-[1.02] hover:opacity-90" : "bg-blue-600 shadow-lg shadow-blue-900/50 hover:bg-blue-500"}`}>
              무료로 시작하기 →
            </a>
            {/* ✅ 기능 다시 보기 → 바이브 코딩 배우기 로 변경 */}
            <a href="/learn"
