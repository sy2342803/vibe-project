...
export default function Workspace() {
  ...
  const handleDeleteHistoryItem = (id: string, e: React.MouseEvent) => {
    ...
    if (planData?.id === id) {
      ...
      setIdea("");
      setCurrentStep(1);
      setProgressDesign(false);
      setProgressCode(false);
      setProgressDeploy(false);
    }
  };
  ...
  return (
    <main
      className={`relative min-h-screen antialiased transition-colors duration-300 ${
        isDark ? "bg-[#0a0a16] text-white" : "bg-slate-50 text-slate-900"
      }`}
    >
      {isDark && (
      ...
      <nav
        className={`sticky top-0 z-50 border-b backdrop-blur-md transition-colors ${
          isDark ? "border-white/5 bg-white/5" : "border-slate-200 bg-white/90"
        }`}
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-4">
            <Link
              href="/"
              className="text-lg font-black tracking-tighter bg-gradient-to-r from-blue-500 to-violet-500 bg-clip-text text-transparent hover:opacity-80 transition"
            >
              🚀 VIBE 바이브코딩
            </Link>
            <Link
          ...
          <div className="flex items-center gap-4">
            {/* 멘토 토글 */}
            <div
              className={`flex rounded-xl border p-0.5 text-xs font-bold ${
                isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-100"
              }`}
            >
              <button
                type="button"
                onClick={() => setMentorTone("kind")}
                className={`px-2 py-1 rounded-lg transition ${
                  mentorTone === "kind" ? "bg-blue-500 text-white" : isDark ? "text-slate-400" : "text-slate-500"
                }`}
              >
                😇 천사 멘토
              </button>
              <button
                type="button"
                onClick={() => setMentorTone("tsundere")}
                className={`px-2 py-1 rounded-lg transition ${
                  mentorTone === "tsundere" ? "bg-amber-600 text-white" : isDark ? "text-slate-400" : "text-slate-500"
                }`}
              >
                🦊 츤데레 선배
              </button>
            ...
            >
              <button
                ...
                className={`p-1.5 rounded-full transition ${
                  !isDark ? "bg-white shadow-sm text-blue-600" : "text-slate-400"
                }`}
              >
                <SunIcon />
              </button>
              <button
                ...
                className={`p-1.5 rounded-full transition ${
                  isDark ? "bg-white/10 shadow-sm text-violet-400" : "text-slate-400"
                }`}
              >
                <MoonIcon />
              </button>
      ...
      <div className="flex mx-auto max-w-7xl min-h-[calc(100vh-69px)]">
        {/* ── 왼쪽 사이드바 ── */}
        <aside
          className={`w-64 border-r shrink-0 hidden md:block p-4 overflow-y-auto space-y-4 ${
            isDark ? "border-white/5 bg-[#0d0d1f]" : "border-slate-200 bg-white"
          }`}
        >
          <div
          ...
          >
            ...
            <button
              ...
              className="text-[11px] px-2 py-1 font-bold bg-blue-500 text-white rounded-md hover:bg-blue-600 transition"
            >
              + 새 기획
            </button>
          </div>
                ...
                >
                  ...
                  <div className="absolute top-2 right-2">
                    ...
                    {activeMenuId === item.id && (
                      <div
                        className={`absolute right-0 mt-1 w-20 rounded-md shadow-lg border text-xs z-50 overflow-hidden ${
                          isDark ? "bg-neutral-900 border-white/10" : "bg-white border-slate-200"
                        }`}
                      >
                        <button
                          onClick={(e) => handleDeleteHistoryItem(item.id, e)}
                          className="w-full text-left px-2 py-1.5 text-red-500 hover:bg-red-500/10 font-bold"
                        >
                          삭제하기
                        </button>
                      </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          {planData && (
            <div
              className={`border-b text-xs py-2.5 px-6 transition-colors ${
                isDark ? "bg-blue-950/20 border-white/5" : "bg-blue-50 border-slate-200"
              }`}
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                ...
                <div
                  className={`flex flex-wrap gap-4 items-center font-medium ${
          ...
          {/* 스텝 탭 네비 */}
          <div
            className={`border-b sticky top-[69px] z-40 transition-colors ${
              isDark ? "border-white/5 bg-[#0f0f23]" : "border-slate-200 bg-white shadow-sm"
            }`}
          >
            ...
            {loading && (
              <div className="my-20 flex flex-col items-center justify-center gap-4 text-center">
                ...
                <p className="text-sm text-slate-400">
                  {mentorTone === "kind"
                    ? "비전공자도 하루 만에 구현할 수 있는 가장 쉬운 방식의 뼈대를 짜는 중입니다."
                    : "형편없는 주석 달기 전에 알아서 굴러가게 아키텍처 뽑고 있으니까 잠깐만 기다려 봐."}
                </p>
              </div>
            )}
              ...
              <div className="space-y-8 animate-fadeIn">
                <div className={`border-b pb-4 ${isDark ? "border-white/10" : "border-slate-200"}`}>
                  <h1 className="text-2xl font-black md:text-3xl tracking-tight">
                    머릿속 아이디어를 하루 만에 현실로 만들어보세요!
                  </h1>
                  <p className={`text-sm mt-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                    코딩을 몰라도 괜찮습니다. 만들고 싶은 서비스의 형태와 종류를 고르고 툭 던져만 주세요.
                  </p>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    ...
                    rows={5}
                    className={`w-full rounded-2xl border p-4 text-base leading-relaxed outline-none focus:ring-2 focus:ring-blue-500 transition ${
                      isDark
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {examples.map((ex) => (
                      <button
                        ...
                        type="button"
                        onClick={() => setIdea(ex)}
                        className={`p-4 rounded-xl border text-left text-xs leading-relaxed transition ${
                          isDark
                            ? "bg-white/5 border-white/10 text-slate-300 hover:border-blue-500"
                            : "bg-white border-slate-200 text-slate-600 hover:border-blue-500 hover:bg-blue-50/50"
                        }`}
                      >
                        {ex}
                      </button>
                ...
                <div className={`pt-6 border-t ${isDark ? "border-white/5" : "border-slate-200"}`}>
                  <span className="text-xs font-black text-slate-400 block mb-3">
                    🔥 오늘 VIBE를 거쳐 간 선배 메이커들의 생생한 한줄 발자취
                  </span>
              ...
              <div className="space-y-6 animate-fadeIn">
                <div className={`border-b pb-4 flex flex-wrap items-center justify-between gap-4 ${isDark ? "border-white/10" : "border-slate-200"}`}>
                  <div>
                    ...
                    <p className={`text-sm mt-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                      선택한 설정:{" "}
                      <span className="text-blue-500 font-bold">{selectedPlatform}</span> |{" "}
                      <span className="text-violet-500 font-bold">{selectedBM}</span>
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                {planData.summary && (
                  <div
                    className={`p-4 rounded-2xl border ${
                      mentorTone === "kind"
                        ? isDark
                          ? "bg-[#12122c] border-blue-500/20"
                          : "bg-blue-50 border-blue-200"
                        : isDark
                        ? "bg-[#1a1424] border-amber-500/20"
                        : "bg-amber-50 border-amber-200"
                    }`}
                  >
                    <span
                      className={`text-xs font-black block mb-1.5 ${
                        mentorTone === "kind"
                    ...
                    >
                      {mentorTone === "kind"
                        ? planData.summary
                        : "솔직히 기획 수준이 갈 길이 멀긴 한데, 그래도 뼈대는 갖췄으니까 눈 똑바로 뜨고 확인해봐."}
                    </p>
                  </div>
                )}
                ...
                <div className={`flex border-b gap-2 overflow-x-auto ${isDark ? "border-white/10" : "border-slate-200"}`}>
                  {[
                    ...
                    <button
                      key={tab.id}
                      type="button"
                      onClick={() => setStep2Tab(tab.id as any)}
                ...
                {/* 탭 콘텐츠 패널 */}
                <div
                  className={`p-4 rounded-2xl border min-h-[250px] ${
                      ...
                      {planData.features?.map((f, i) => (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-xs font-black text-blue-500 bg-blue-500/10 px-2 py-0.5 rounded">
                              기능 {i + 1}
                            </span>
                          ...
                          >
                            <span className="text-[10px] font-bold text-blue-500 block mb-1.5">
                              🚨 AI 코딩 메이킹이 끝난 후 눈으로 직접 확인할 체크리스트
                            </span>
                            <ul
                              className={`space-y-1 text-xs list-disc pl-4 ${
                  ...
                  {step2Tab === "stories" && (
                    <div className="space-y-2">
                      {planData.userStories?.map((story, idx) => (
                        ...
                        >
                          <span
                            className={`h-5 w-5 shrink-0 rounded-full flex items-center justify-center font-bold text-slate-500 ${
                              isDark ? "bg-white/10" : "bg-slate-200"
                            }`}
                          >
                            {idx + 1}
                    ...
                    <div className="grid gap-4 md:grid-cols-2">
                      {planData.screens?.map((scr, i) => (
                        <div
                          key={i}
                          className={`p-4 border rounded-xl ${
                            isDark ? "border-white/10 bg-neutral-900" : "border-slate-200 bg-white"
                          }`}
                        >
                    ...
                    <div className="space-y-4 text-xs">
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        ...
                        >
                          ...
                          <p className={isDark ? "text-slate-400 leading-relaxed" : "text-slate-500 leading-relaxed"}>
                            깃허브 저장소와 단 3번의 클릭으로 연결됩니다. 결제 카드 등록 없이도 전 세계 누구나 접속할 수 있는 공짜 주소(.vercel.app)를 즉시 발급해 줍니다.
                          </p>
                        </div>
                        <div
                          className={`p-4 border rounded-xl ${
                        ...
                        >
                          ...
                          <p className={isDark ? "text-slate-400 leading-relaxed" : "text-slate-500 leading-relaxed"}>
                            엑셀처럼 쓸 수 있는 웹 데이터베이스 상자입니다. 동시 접속자 수백 명이 들어와도 끄떡없는 용량을 평생 비용 없이 무료 범위 안에서 제공합니다.
                          </p>
                        </div>
                      </div>
                      <div className="p-3 bg-blue-500/5 rounded-xl border border-blue-500/20 text-[11px] text-center text-slate-500">
                        📢 <strong>꿀팁:</strong> Cursor AI 에디터에게{" "}
                        <strong>"이 프로젝트를 Vercel에 무료로 올리는 구조로 세팅해줘"</strong> 라고 치면 세팅까지 알아서 다 해줍니다.
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center pt-4">
                  <button
                    type="button"
                    onClick={handleGoToNewPlan}
                    className={`text-xs font-bold underline ${isDark ? "text-slate-400" : "text-slate-500 hover:text-slate-700"}`}
                  >
                    ← 새 기획 작성화면 이동
                  </button>
                  <button
                    ...
                    className="px-5 py-2.5 rounded-xl text-xs font-black bg-blue-600 text-white hover:bg-blue-700 shadow-md"
                  >
            ...
            {!loading && planData && currentStep === 3 && (
              <div className="space-y-6 animate-fadeIn">
                <div className={`border-b pb-4 ${isDark ? "border-white/10" : "border-slate-200"}`}>
                  ...
                  <p className={`text-sm mt-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                    원하는 AI 제작 도구를 선택하고 버튼을 누르세요. 복사한 텍스트를 해당 AI에게 던지면 서비스가 하루 만에 구축됩니다.
                  </p>
                </div>
                ...
                <div className={`flex border-b gap-2 overflow-x-auto ${isDark ? "border-white/10" : "border-slate-200"}`}>
                  <button
                    type="button"
                    onClick={() => setStep3Tab("ui")}
                    className={`px-4 py-2 text-xs font-bold border-b-2 transition whitespace-nowrap ${
                      step3Tab === "ui" ? "border-blue-500 text-blue-500" : isDark ? "border-transparent text-slate-400" : "border-transparent text-slate-400 hover:text-slate-700"
                    }`}
                  >
                    🎨 1단계: 테마 디자인 (v0.dev 용)
                  </button>
                  <button
                    type="button"
                    onClick={() => setStep3Tab("ide")}
                    className={`px-4 py-2 text-xs font-bold border-b-2 transition whitespace-nowrap ${
                      step3Tab === "ide" ? "border-violet-500 text-violet-500" : isDark ? "border-transparent text-slate-400" : "border-transparent text-slate-400 hover:text-slate-700"
                    }`}
                  >
                    💻 2단계: 기능 연결 (Cursor 에디터 용)
                  </button>
                  <button
                    type="button"
                    onClick={() => setStep3Tab("db")}
                    className={`px-4 py-2 text-xs font-bold border-b-2 transition whitespace-nowrap ${
                      step3Tab === "db" ? "border-emerald-500 text-emerald-500" : isDark ? "border-transparent text-slate-400" : "border-transparent text-slate-400 hover:text-slate-700"
                    }`}
                  >
                    🗄️ 3단계: 정보 창고 (Supabase 용)
                  </button>
                </div>

                {/* 탭 패널 */}
                <div
                  className={`p-5 rounded-2xl border min-h-[200px] ${
                    isDark ? "bg-white/5 border-white/10" : "bg-white border-slate-200"
                  }`}
                >
                  {step3Tab === "ui" && (
                    <div className="space-y-3">
                      <div className="text-[11px] text-blue-500 font-bold mb-2">
                        💡 사용법: 아래 박스의 프롬프트를 복사한 뒤 v0.dev 혹은 Bolt.new 사이트의 입력창에 그대로 붙여넣으세요!
                      </div>
                      {planData.prompts?.ui?.map((p, i) => (
                        <div
                          key={i}
                          className={`flex justify-between items-start gap-4 p-4 border rounded-xl ${
                        ...
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => triggerCopy(p)}
                            className={`shrink-0 text-xs px-2.5 py-1.5 font-bold border rounded transition ${
                              isDark ? "bg-neutral-800 border-white/10 text-blue-400" : "bg-white border-slate-200 text-blue-500 hover:bg-blue-50"
                            }`}
                          >
                  ...
                  {step3Tab === "ide" && (
                    <div className="space-y-3">
                      <div className="text-[11px] text-violet-500 font-bold mb-2">
                        💡 사용법: 아래 명령어를 복사하여 Cursor 프로그램 우측의 AI 채팅창(Ctrl+L)에 넣으세요!
                      </div>
                      {planData.prompts?.ide?.map((p, i) => (
                        ...
                        >
                          ...
                          <button
                            ...
                            onClick={() => triggerCopy(p)}
                            className={`shrink-0 text-xs px-2.5 py-1.5 font-bold border rounded transition ${
                              isDark ? "bg-neutral-800 border-white/10 text-violet-400" : "bg-white border-slate-200 text-violet-500 hover:bg-violet-50"
                            }`}
                          >
                  ...
                  {step3Tab === "db" && (
                    <div className="space-y-3">
                      <div className="text-[11px] text-emerald-500 font-bold mb-2">
                        💡 사용법: 데이터 보관함 스크립트입니다.
                      </div>
                      <div className="flex justify-between items-start gap-4 p-4 border rounded-xl bg-slate-900 text-emerald-400 font-mono text-xs overflow-x-auto">
                        ...
                        <button
                          type="button"
                          onClick={() => triggerCopy(planData.prompts?.db)}
                ...
                {/* 주의사항 */}
                <div className="p-4 rounded-xl border border-amber-500/20 bg-amber-50 text-xs text-slate-600 leading-relaxed">
                  <span className="font-bold text-amber-600 block mb-1">⚠️ 복붙 전 꼭 읽어두면 주머니와 멘탈에 좋은 가이드</span>
                  AI 도구 화면에 빨간 불이 아니라{" "}
                  <strong className="text-amber-600">노란색 느낌표 경고(Warning)</strong>가 뜨는 건 컴퓨터가 혼자 잔소리하는 거라 가볍게 무시하셔도 완벽하게 작동합니다! 또한 무료 범위 내에서 카드를 등록하라는 알림이 오더라도 당황하지 마시고 창을 닫은 후 무료 등급으로 계속 사용하시면 지갑 방어가 완료됩니다.
                </div>
            ...
            {!loading && planData && currentStep === 4 && (
              <div className="space-y-6 animate-fadeIn">
                <div className={`border-b pb-4 ${isDark ? "border-white/10" : "border-slate-200"}`}>
                  ...
                  <p className={`text-sm mt-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                    바이브코딩 도중 에러가 나거나 막힐 때 도움을 요청하세요. (현재 모드:{" "}
                    <span className="text-amber-500 font-bold">
                      {mentorTone === "kind" ? "👼 천사 코치" : "🦊 츤데레 선배"}
                    </span>
                    )
                  </p>
                </div>
                ...
                <div className={`grid grid-cols-2 md:grid-cols-4 gap-2 border-b pb-2 ${isDark ? "border-white/5" : "border-slate-200"}`}>
                  {[
                    ...
                    <button
                      key={tool.id}
                      type="button"
                      onClick={() => setStep4Tool(tool.id as any)}
                ...
                >
                  ...
                  {step4Tool === "error" && (
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-xs font-black mb-1 text-red-500">
                          화면에 뜬 무서운 영어 에러 문장을 그대로 복사해서 넣어주세요
                        </h3>
                        <textarea
                          value={errorInput}
                          ...
                          rows={3}
                          className={`w-full text-xs font-mono p-3 rounded-xl border outline-none focus:ring-2 focus:ring-blue-500 ${
                            isDark
                              ? "bg-neutral-900 border-white/10 text-white placeholder:text-slate-600"
                              : "bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400"
                          }`}
                        />
                        <button
                          type="button"
                          onClick={() => handleSubToolSubmit("error", errorInput)}
                          disabled={errorLoading || !errorInput.trim()}
                        ...
                        >
                          ...
                          <div className={`pt-2 border-t flex justify-between items-center ${isDark ? "border-white/10" : "border-slate-200"}`}>
                            <span className="text-[10px] text-slate-400">
                              AI에게 줄 치트키: <strong>{errorResult.prompt}</strong>
                            </span>
                            <button
                              type="button"
                              onClick={() => triggerCopy(errorResult.prompt)}
                              className="text-blue-500 font-bold shrink-0"
                            >
                              {copiedText === errorResult.prompt ? "복사됨!" : "명령어 복사"}
                            </button>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-xs font-black mb-1">
                          내가 적은 러프한 질문을 AI가 가장 좋아하는 똑똑한 질문으로 리모델링합니다
                        </h3>
                        <textarea
                          value={refinerInput}
                          onChange={(e) => setRefinerInput(e.target.value)}
                          placeholder="예) 로그인 화면 하나 이쁘게 리액트로 짜줘"
                          rows={3}
                          className={`w-full text-xs p-3 rounded-xl border outline-none focus:ring-2 focus:ring-violet-500 ${
                            isDark
                              ? "bg-neutral-900 border-white/10 text-white placeholder:text-slate-600"
                              : "bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400"
                          }`}
                        ...
                        <button
                          type="button"
                          onClick={() => handleSubToolSubmit("prompt", refinerInput)}
                          disabled={refinerLoading || !refinerInput.trim()}
                      ...
                      {refinerResult && (
                        <div className="grid md:grid-cols-2 gap-3 text-xs">
                          ...
                          >
                            <span className="text-slate-400 block mb-1">내가 쓴 글</span>
                            {refinerResult.original}
                          </div>
                          <div className="p-3 border border-violet-500/30 rounded-xl bg-violet-500/5 relative">
                            <span className="text-violet-500 font-bold block mb-1">AI 찰떡 저격 글</span>
                            {refinerResult.improved}
                            <button
                              type="button"
                              onClick={() => triggerCopy(refinerResult.improved)}
                              className="absolute bottom-2 right-2 text-[10px] bg-neutral-800 text-white px-2 py-1 rounded"
                            >
                  ...
                  {step4Tool === "mock" && (
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-xs font-black mb-1">
                          화면을 가득 채워줄 임시 가짜 정보 데이터 샘플 상자입니다
                        </h3>
                        <input
                          type="text"
                          value={mockInput}
                          ...
                          className={`w-full text-xs p-3 rounded-xl border outline-none focus:ring-2 focus:ring-emerald-500 ${
                            isDark
                              ? "bg-neutral-900 border-white/10 text-white placeholder:text-slate-600"
                              : "bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400"
                          }`}
                        ...
                        <button
                          type="button"
                          onClick={() => handleSubToolSubmit("mock", mockInput || idea)}
                      ...
                      {mockResult && (
                        <div className="p-3 border rounded-xl bg-slate-900 text-emerald-400 font-mono text-xs relative overflow-x-auto">
                          ...
                          <button
                            type="button"
                            onClick={() => triggerCopy(mockResult)}
                            className="absolute top-2 right-2 text-[10px] bg-slate-800 text-white px-2 py-1 rounded"
                          >
                  ...
                  {step4Tool === "commit" && (
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-xs font-black mb-1">
                          방금 무슨 작업을 완료했는지 한글로 적어주시면 멋진 깃 메시지로 바꿔드려요
                        </h3>
                        <input
                          type="text"
                          value={commitInput}
                          ...
                          className={`w-full text-xs p-3 rounded-xl border outline-none focus:ring-2 focus:ring-amber-500 ${
                            isDark
                              ? "bg-neutral-900 border-white/10 text-white placeholder:text-slate-600"
                              : "bg-slate-50 border-slate-200 text-slate-900 placeholder:text-slate-400"
                          }`}
                        ...
                        <button
                          type="button"
                          onClick={() => handleSubToolSubmit("commit", commitInput)}
                          disabled={commitLoading || !commitInput.trim()}
                      ...
                      {commitResult && (
                        <div
                          className={`p-3 border rounded-xl flex justify-between items-center text-xs font-mono ${
                            isDark ? "bg-white/5 border-white/10" : "bg-slate-50 border-slate-200"
                          }`}
                        >
                          ...
                          <button
                            type="button"
                            onClick={() => triggerCopy(commitResult)}
                            className={`font-bold text-[11px] ${isDark ? "text-slate-400" : "text-slate-500 hover:text-slate-700"}`}
                          >
      ...
      {planData && (
        <div style={{ position: "absolute", top: "-9999px", left: "-9999px" }}>
          <div
            ref={reportRef}
            style={{ width: "800px", padding: "50px", backgroundColor: "#ffffff", color: "#1e293b", fontFamily: "sans-serif" }}
          >
            <div style={{ borderBottom: "3px solid #2563eb", paddingBottom: "15px", marginBottom: "35px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "#64748b", borderTop: "1px solid #e2e8f0", paddingTop: "10px" }}>
                <div>
                  <strong>선택 구동 플랫폼:</strong> {selectedPlatform} | <strong>선택 수익 모델:</strong> {selectedBM}
                </div>
              </div>
            </div>
                  ...
                  {planData.features?.map((f, idx) => (
                    <tr key={idx} style={{ borderBottom: "1px solid #e2e8f0" }}>
                      <td style={{ padding: "8px", fontWeight: "bold" }}>
                        {f.name}
                        <br />
                        <span style={{ fontSize: "10px", fontWeight: "normal", color: "#64748b" }}>{f.desc}</span>
                      </td>
                      <td style={{ padding: "8px" }}>
                        <ul style={{ margin: 0, paddingLeft: "15px" }}>
                          {f.criteria?.map((cr, cidx) => (
                            <li key={cidx}>{cr}</li>
                          ))}
                        </ul>
                      </td>
